...
int main(int argc, char** argv) {
    ...
    int c;
    int option_index = 0;
    while ((c = getopt_long(argc, argv, "p:c:t:b:h:",
    ...
    printf("[-] Config: host=%s:%d, bandwidth=%s, time=%ds\n", host_, port_, size_hrdr, time_);
    struct Dest dest;
    ...
    while (1) {
        sleep_time = SECOND;
        if (lastTime.tv_nsec && lastTime.tv_sec) {
            clock_gettime(CLOCK_MONOTONIC_RAW, &currentTime);
            sleep_time -= (currentTime.tv_sec - lastTime.tv_sec)* 1e6
                + (currentTime.tv_nsec - lastTime.tv_nsec) / 1e3;
